...
///
#include "stdafx.h"
#include "TimeEditCtrl_MS.h"

///
/// @brief
///     时间串的格式、限定字符数及时间元素数量
///
#define TIME_EDIT_MS_FORMAT_STR _T("%04d-%02d-%02d %02d:%02d:%02d.%03d")
#define TIME_EDIT_MS_STR_SIZE   23
#define TIME_EDIT_MS_ELEM_NUM   7

///
/// @brief
///     时间串中各时间值的偏移地址及长度
///
const int k_TimePos[TIME_EDIT_MS_ELEM_NUM] = {0,5,8,11,14,17,20};
const int k_TimeLen[TIME_EDIT_MS_ELEM_NUM] = {4,2,2,2,2,2,3};
...
///
#define TIME_EDIT_MS_CMD_BACKSPACE  32799

///
/// @brief
///     CIEC61850TimeEditCtrl
///
BEGIN_MESSAGE_MAP(CTimeEditCtrl_MS, CWnd)
	...
	ON_COMMAND(TIME_EDIT_MS_CMD_BACKSPACE, &CTimeEditCtrl_MS::OnBtnBackspace)
	ON_UPDATE_COMMAND_UI(TIME_EDIT_MS_CMD_BACKSPACE, &CTimeEditCtrl_MS::OnUpdateBtnBackspace)
	ON_WM_CAPTURECHANGED()
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
END_MESSAGE_MAP()

CTimeEditCtrl_MS::CTimeEditCtrl_MS ()
: m_pParentWnd(NULL)
, m_clrText(RGB(0, 0, 0))
...
///     设置背景颜色
///
void  CTimeEditCtrl_MS::SetBkColor (COLORREF color)
{
	m_bkColor = color;
...
///     设置文字颜色
///
void  CTimeEditCtrl_MS::SetTextColor (COLORREF color)
{
	m_clrText = color;
}
...
///     设置边框颜色
///
void  CTimeEditCtrl_MS::SetBorderColor (COLORREF color)
{
	m_clrBorder = color;
}
...
///     设置文字字体，默认为父窗口字体
///
void  CTimeEditCtrl_MS::SetTextFont(CFont* pFont)
{
	m_pTextFont = pFont;
}
...
///     
///
void CTimeEditCtrl_MS::Str2SysTime(const CString &str, SYSTEMTIME &time)
{
	...
	time.wMilliseconds = (WORD)_ttoi(str.Mid(k_TimePos[6],k_TimeLen[6]));
}
...
///     
///
void CTimeEditCtrl_MS::SysTime2Str(const SYSTEMTIME &time, CString &str)
{
	str.Format(TIME_EDIT_MS_FORMAT_STR,time.wYear,time.wMonth,time.wDay,
...
///     
///
int CTimeEditCtrl_MS::GoModal(CWnd *pParentWnd, const CRect &rt, SYSTEMTIME &time)
{
	if (::IsWindow(m_hWnd)) 
	{
		ASSERT(FALSE);
...
///     
///
void  CTimeEditCtrl_MS::Create(CWnd *pParentWnd, const CRect &rt, const SYSTEMTIME &time)
{
	ASSERT(pParentWnd);
	...
	// 设置最大字符数
	SetLimitText(TIME_EDIT_MS_STR_SIZE);
...
///     隐藏Inplace编辑窗口
///
void  CTimeEditCtrl_MS::HideWindow ()
{
	if (m_hWnd)
	{
...
///     确认用户输入修改, 通知父窗口
///
void  CTimeEditCtrl_MS::ApplyInput ()
{
	if ( m_bGoModal )
	{
		EndModalLoop(IDOK);
	...
	{
		// 向父窗口发送消息
		::SendMessage(m_pParentWnd->m_hWnd,WM_TIME_EDIT_MS_APPLY,0,0);
		// 销毁窗口
		DestroyWindow();
	}
}
...
///     用户取消修改
///
void  CTimeEditCtrl_MS::CancelInput ()
{
	if ( m_bGoModal )
	{
	...
	{
		// 向父窗口发送消息
		::SendMessage(m_pParentWnd->m_hWnd,WM_TIME_EDIT_MS_CANCLE,0,0);
		// 销毁窗口
		DestroyWindow();
	}
}
...
///
void  CTimeEditCtrl_MS::OnEnKillfocus ()
{
	if (m_bCancel)
	{
		CancelInput();
...
///
UINT  CTimeEditCtrl_MS::OnGetDlgCode ()
{
...
///     截获ESC, 回车等键, 确认用户修改完成
///
void  CTimeEditCtrl_MS::OnChar (UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar)
	{
	case VK_BACK:
...
void  CTimeEditCtrl_MS::OnKeyDown (UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CEdit::OnKeyDown(nChar, nRepCnt, nFlags);
}

void  CTimeEditCtrl_MS::OnSysKeyDown (UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CEdit::OnSysKeyDown(nChar, nRepCnt, nFlags);
}

void CTimeEditCtrl_MS::OnCaptureChanged(CWnd *pWnd)
{   
	if (m_hWnd)
	{
		m_bCancel = true;
...
BOOL CTimeEditCtrl_MS::OnEraseBkgnd(CDC* pDC)
{
...
void CTimeEditCtrl_MS::OnPaint ()
{
#ifdef IEC61850_TIME_EDIT_DOUBLEBUFFER
...
///
BOOL  CTimeEditCtrl_MS::PreTranslateMessage (MSG* pMsg)
{
...
///     修改控件颜色
///
HBRUSH  CTimeEditCtrl_MS::CtlColor(CDC* pDC, UINT /*nCtlColor*/)
{
	// TODO:  在此更改 DC 的任何属性
...
///      替代键盘上的Backspace
///
void CTimeEditCtrl_MS::OnBtnBackspace()
{
	PostMessage(WM_CHAR, VK_BACK, 0);
}
void CTimeEditCtrl_MS::OnUpdateBtnBackspace(CCmdUI *pCmdUI)
{
	pCmdUI->Enable(1);
}
...
///     5 - 秒
///
int CTimeEditCtrl_MS::GetTimeTypeInCursorPos(const int pos)
{
	for ( int i = 1; i < TIME_EDIT_MS_ELEM_NUM; i++ )
	{
		if ( pos < k_TimePos[i] )
		{
			return i-1;
	...
	return (TIME_EDIT_MS_ELEM_NUM-1);
}
...
///     
///
void CTimeEditCtrl_MS::TimeStr2IntArr(const CString &timeStr, int *timeArr , const int arrSize)
{
	CString str;
	for ( int i = 0; i < arrSize; i++ )
	{
		if ( i >= TIME_EDIT_MS_ELEM_NUM )
			break;
...
///     TRUE - 溢出; FALSE - 没溢出
///
BOOL CTimeEditCtrl_MS::IsOverFlow(const int timeType, const int *timeArr)
{
	switch(timeType)
	{
	case 0:
...
///     
///
void CTimeEditCtrl_MS::CorrectTimeStr(CString &str)
{
	for ( int i = 0; i < TIME_EDIT_MS_ELEM_NUM; i++ )
	{
...
///     
///
void  CTimeEditCtrl_MS::CorrectYear(CString &str)
{
	// 判断年份是否<1970
	...
	{
		CString s =str;
		str.Format(_T("%s%s"),_T("1970"),s.Right(s.GetLength()-k_TimeLen[0]));
		SetWindowText(str);
	}
}
...
///
void  CTimeEditCtrl_MS::KeyBackDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
///     TAB按键响应，操作结束后直接返回
///
void  CTimeEditCtrl_MS::KeyTabDown()
{
	int rightPos = HIWORD(GetSel());  // 获取选中文本右边光标相对于控件的位置
	int timeType = GetTimeTypeInCursorPos(rightPos);
	...
	// 选中下一项时间项
	timeType++;
	if( timeType > TIME_EDIT_MS_ELEM_NUM - 1 )
		timeType = 0;
...
///     ESC按键响应，操作结束后直接返回
///
void  CTimeEditCtrl_MS::KeyEscapeDown()
{
	// 用户取消修改
	m_bCancel = true;
	// 恢复为原始文本
	CString str;
	str.Format(TIME_EDIT_MS_FORMAT_STR,m_tmPreTime.wYear,m_tmPreTime.wMonth,m_tmPreTime.wDay,
		m_tmPreTime.wHour,m_tmPreTime.wMinute,m_tmPreTime.wSecond,m_tmPreTime.wMilliseconds);
	SetWindowText(str);
...
///     回车键按键响应
///
void  CTimeEditCtrl_MS::KeyReturnDown()
{
	// 修正多余的' '为'0'
	CString timeStr;
	GetWindowText(timeStr);
...
///     数字键按键响应
///
void  CTimeEditCtrl_MS::KeyNumDown(UINT nChar)
{
	if ( nChar < '0' && nChar > '9' )
		return;
	...
	{// 未选中字符，且时间值编辑区域存在' '的情况
		...
		str1.Format(_T("%s%s%s"),timeStr.Left(k_TimePos[timeType]),str2,
			timeStr.Right(timeStr.GetLength()-k_TimePos[timeType]-k_TimeLen[timeType]));
		// 判断编辑后的时间值是否溢出，是则返回
		int timeArr[TIME_EDIT_MS_ELEM_NUM];
		TimeStr2IntArr(str1,timeArr,TIME_EDIT_MS_ELEM_NUM);
		...
		// 更新结果
		SetWindowText(str1);
		if ( str1[k_TimePos[timeType]] != ' ' && timeType < TIME_EDIT_MS_ELEM_NUM-1 )
		{
			// 判断年份是否<1970
			if( timeType == 0 )
			{
		...
		else
		{
			SetSel(rightPos,rightPos);
		      ...
		      timeStr[k_TimePos[timeType]] != ' ' && timeType < TIME_EDIT_MS_ELEM_NUM-1 )
	{
		// 判断年份是否<1970
		if( timeType == 0 )
		{
...
///     左方向键按键响应
///
void  CTimeEditCtrl_MS::KeyMoveLeft()
{
		...
		(leftPos == k_TimePos[timeType]) )
	{
		...
		if ( timeType == 0 )
		{
			...
			timeType = sizeof(k_TimePos)/sizeof(int) - 1;
		}
		else
		{
			timeType--;
...
///     右方向键按键响应
///
void  CTimeEditCtrl_MS::KeyMoveRight()
{
	int rightPos = HIWORD(GetSel());  // 获取选中文本右边光标相对于控件的位置
	int timeType = GetTimeTypeInCursorPos(rightPos);
	CString timeStr;
...
///     上方向键按键响应
///
void  CTimeEditCtrl_MS::KeyMoveUp()
{
	int rightPos = HIWORD(GetSel());  // 获取选中文本右边光标相对于控件的位置
	int timeType = GetTimeTypeInCursorPos(rightPos);
	CString timeStr;
	...
	// 获取编辑后的时间值
	GetWindowText(timeStr);
	TimeStr2IntArr(timeStr,timeArr,TIME_EDIT_MS_ELEM_NUM);
	timeArr[timeType]++;
	timeStr.Format(TIME_EDIT_MS_FORMAT_STR,timeArr[0],timeArr[1],timeArr[2],timeArr[3],timeArr[4],timeArr[5],timeArr[6]);
	// 判断溢出
	if ( IsOverFlow(timeType,timeArr) == TRUE )
		return;
	if ( timeType == 1 && IsOverFlow(2,timeArr) == TRUE )
...
///     下方向键按键响应
///
void  CTimeEditCtrl_MS::KeyMoveDown()
{
	int rightPos = HIWORD(GetSel());  // 获取选中文本右边光标相对于控件的位置
	int timeType = GetTimeTypeInCursorPos(rightPos);
	CString timeStr;

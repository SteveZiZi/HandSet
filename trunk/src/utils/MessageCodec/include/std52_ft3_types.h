...
///
/// @brief
///     定义标准FT3采样值报文（52字节版本,含CRC）帧编解码过程中使用的数据类型
///
/// @note
///
...
///
/// @brief
///     标准FT3采样值报文（52字节版本,含CRC）每帧报文长度为26words
///
#define STD52_FT3_MSG_WORD       26

///
/// @brief
///     标准FT3采样值报文（52字节版本,含CRC）的应用服务数据单元（ASDU）中除采样值信息外的公共信息
///
typedef struct
{
...
///
/// @brief
///     标准FT3采样值报文（52字节版本,含CRC）的应用服务数据单元（ASDU）中除采样值信息外的公共信息的偏移地址信息
///
typedef struct
{

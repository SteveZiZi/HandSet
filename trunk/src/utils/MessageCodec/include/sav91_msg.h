...
///
/// @brief
///     IEC61850-9-1报文帧的编解码接口及接口对象实例的创建和释放接口
///
/// @note
///
...
///    |               |           |                                                          |
///
#ifndef SAV91_MESSAGE_H_

#define SAV91_MESSAGE_H_

#include "etypes.h"
#include "sav91_types.h"

///
/// @brief
///     IEC61850-9-1采样值报文的编解码接口
///
class Sav91Msg
{
public:
	...
	///     true - 成功； false - 失败
	///
	virtual bool GetASDUPublicInfo(const size_t idxSeq, SAV91_ASDU_PUBLIC &asduPublic) = 0;
	...
	///     true - 成功； false - 失败
	///
	virtual bool GetStatusWordsInASDU(const size_t idxSeq, uint16_t &statusWord1, uint16_t &statusWord2) = 0;
	
	///
	/// @brief
	///     获取协议定义的ASDU中的采样值数据部分的通道数
	///
	/// @return
	///     协议定义的ASDU的采样值数据部分的通道数
	///
	virtual size_t GetASDUDefaultSmpsNum() = 0;
	...
	///     true - 成功； false - 失败
	///
	virtual bool GetASDUPublicInfoOff(const size_t idxSeq, SAV91_ASDU_PUBLIC_OFFSET &asduPublicOff) = 0;
	...
	///     true - 成功； false - 失败
	///
	virtual bool AddAnASDU(const SAV91_ASDU_PUBLIC &asduPublic, const uint16_t *smpValbuf, const size_t bufSize) = 0;
};

...
const WCHAR *GetFileName(const WCHAR *strFilePath)
{
    int len=(int)wcslen(strFilePath);
    len--;
    assert(strFilePath!=NULL);
    assert(strFilePath[0]!=0);

    const WCHAR *p=strFilePath+len;

    while(*p !=L'\\' && p!=strFilePath )
    {
        p--;
    }

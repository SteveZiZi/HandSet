...
Net_Base_c::Net_Base_c(HANDLE hDevice, UINT16 nDeviceItem) 
{
    ...
    m_hStopTxEvent  = INVALID_HANDLE_VALUE;
...
BOOL Net_Base_c::Init(WCHAR *pNotifyEvtName, WCHAR *pMapFileName, UINT32 nMapFileLen, LPTHREAD_START_ROUTINE lpStartAddress)
{
    ...
    m_hStopTxEvent  = CreateEvent(NULL, TRUE, FALSE, NULL);
...
bool Net_Base_c::SetUtcTime()
{
    ...
    CString strTime = L"2000-07-26 09:50:50";
    swscanf_s(strTime, _T("%04d-%02d-%02d %02d:%02d:%02d"), &nYear, &nMonth, &nDay, &nHour, &nMin, &nSec);
...
/// @brief
///     设置发送报文的相关参数
/// @param[IOCTRL_FIBER_IN_PARAM_t & tMuSvTxParam]
///     发送参数
/// @param[bool bFromFile]
///     是否要从文件中读取报文
/// @return
///
///
...
//                 MessageBox(_T("the File is not GOOSE Frame\r\n"));
            ...
            if(j == 0)
            {
                if(GetUtcTime())
                {
                    memcpy(&pMuTxData[0], &m_tUtcTime, sizeof(UTC_TIME_t));
                }
                
            }
            
            /*UTC_TIME_t utc;
            memcpy(&utc, (UTC_TIME_t*)&pMuTxData[8], sizeof(UTC_TIME_t));*/
...
{
    IOCTRL_IN_PARAM_t tInParam;
    memset(tInParam.uInBuf, 0, sizeof(tInParam.uInBuf));
...
///
///
bool _SMV_92_c::GetUtcTime()
{
    return DeviceIoControl(m_hDevice, IOCTRL_GET_UTC_TIME,  NULL, NULL , &m_tUtcTime, sizeof(UTC_TIME_t),NULL,NULL) ;
}
            ...
            if(FALSE == bSmvRecv)
            {
                ...
                if(pEth->IsIrigB())
                {
                    ::PostMessage(AfxGetApp()->GetMainWnd()->m_hWnd, 
                        WM_SEND_BCODE, 1, 1);
                    Sleep(3000);
 //                   AfxMessageBox(L"send B");
                }
                ::PostMessage(AfxGetApp()->GetMainWnd()->m_hWnd, 
                    WM_CHANGE_TX, 1, 1);

            }
            //            pEth->m_pFiberProcFun[pEth->m_RxStyle](pArg);

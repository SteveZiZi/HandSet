...
bool Net_Base_c::SetUtcTime()
{
    ...
    tUtcTime.uSecondSinceEpoch = (UINT32)tm.GetTime();

    tUtcTime.uFractionOfSecond = 0;

    if (DeviceIoControl(m_hDevice, IOCTRL_SET_UTC_TIME, &tUtcTime, sizeof(UTC_TIME_t), NULL, NULL, NULL, NULL) == FALSE) {
        RETAILMSG(1, (_T("IOCTRL_SET_UTC_TIME failed")));
        return FALSE;
    } 
  
    if(FALSE == DeviceIoControl(m_hDevice, IOCTRL_GET_UTC_TIME, NULL,NULL, &tUtcTime, sizeof(UTC_TIME_t),NULL,NULL) )
    {
        RETAILMSG(ZONE_ERROR, (_T("getutc failed\r\n")));
    }
...
bool _SMV_92_c::MuTx()
{
 ...
 //       MessageBox(L"DeviceIoControl:IOCTRL_SEND_HEADER failed");
    ...
    tInParam.uDeviceItem = m_tMuSvTxParam.uDeviceItem;
...
bool _SMV_92_c::VertifyBcodeTime()
{
    ...
    RETAILMSG(1, (_T("T1:%d T2:%d\r\n"), tUtcTime.uSecondSinceEpoch, m_tUtcTime.uSecondSinceEpoch));

    //验证UTC时间误差是否在1S内
    if(tUtcTime.uSecondSinceEpoch == m_tUtcTime.uSecondSinceEpoch)
    {
        return TRUE;
    }
    else if(abs(tUtcTime.uSecondSinceEpoch - m_tUtcTime.uSecondSinceEpoch) > 1)
    {
        return FALSE;
    }
    else if(tUtcTime.uSecondSinceEpoch < m_tUtcTime.uSecondSinceEpoch)
    {
        return (m_tUtcTime.uFractionOfSecond < tUtcTime.uFractionOfSecond);
    }
    else
    {
        return (m_tUtcTime.uFractionOfSecond > tUtcTime.uFractionOfSecond);
...
bool _SMV_92_c::SetCurSyncMode(SYNC_MODE_e eSyncMode)
{
    if (DeviceIoControl(m_hDevice, IOCTRL_SET_SYNCTIME_MODE, &eSyncMode, sizeof(SYNC_MODE_e), NULL, NULL, NULL, NULL) == FALSE) {
        RETAILMSG(1, (_T("IOCTRL_SET_SYNCTIME_MODE\r\n")));
        return FALSE;
    }
    return TRUE;
}

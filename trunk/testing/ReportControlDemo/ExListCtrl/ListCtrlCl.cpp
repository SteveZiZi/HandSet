...
void CListCtrlCl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
    switch (nChar)
    {
    case VK_DOWN:
        {
            int  selIndex = GetNextItem( -1, LVNI_SELECTED );
            if (selIndex != -1)
            {
                int  nextIndex = selIndex + 1;
                if (nextIndex < GetItemCount())
                {
                    SetItemState(selIndex, 0, LVIS_SELECTED);
                    SetItemState(nextIndex, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);   //选中行
                    SetSelectionMark(nextIndex);
                    EnsureVisible(nextIndex, 0);
                }
            }
        }
        break;

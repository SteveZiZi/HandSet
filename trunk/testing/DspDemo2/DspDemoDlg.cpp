...
CDspDemoDlg::CDspDemoDlg(CWnd* pParent /*=NULL*/)
	...
	, cpu_info(_T(""))
	, dsp_speed(0)
    , str_time_dspconfig(_T(""))
    , str_time_dspinfo(_T(""))
    , str_time_smv1calc(_T(""))
...
void CDspDemoDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT_DSPCONFIG, str_time_dspconfig);
    DDX_Text(pDX, IDC_EDIT_DSPINFO, str_time_dspinfo);
    DDX_Text(pDX, IDC_EDIT_SMV10CALC, str_time_smv10calc);
    DDX_Text(pDX, IDC_EDIT_SMV1CALC, str_time_smv1calc);
...
void CDspDemoDlg::UpdateDspData()
{
    ...
    str_dsp = "";
    str_dsp += _T("---------------接收数据计数---------------\r\n");
    dataCount = dspDataSet.GetDataCount();
    str.Format(_T("data count:%d\r\n"),dataCount); 
    str_dsp += str;

    str_dsp += _T("---------------接收数据数量差---------------\r\n");
    str.Format(_T("data count:%d\r\n"),dataCount-lastDataCount); 
    str_dsp += str;
    ...
    for(int i=0; i<30; i++)
    {        
        str.Format(_T("effValue[%d]: %f, %f\r\n"),i,pSmvCalcData->fRmsValue[i], pSmvCalcData->fHarmAngle[i][0] - pSmvCalcData->fHarmAngle[0][0]); 
        str_dsp2 += str;
    }
